...
import {
  ...
  SortEvent,
} from './dailyuser-sortable.directive';
import { DailyUser } from './dailyuser.model';
import { DailyUserService } from './dailyuser.service';
import * as XLSX from 'xlsx';

@Component({
  selector: 'app-dailyuser',
  templateUrl: './dailyuser.component.html',
  styleUrls: ['./dailyuser.component.scss'],
  providers: [DailyUserService, DecimalPipe],
})

/**
 * DailyUser Component
 */
export class DailyUserComponent implements OnInit {
  // bread crumb items
  ...
  hideme: boolean[] = [];
  tables$!: Observable<DailyUser[]>;
  total$!: Observable<number>;

  company: any;
  getCompanys!: Observable<any>;
  companysCollection!: AngularFirestoreCollection<DailyUser>;

  tableData!: DailyUser[];
  totalData!: number;

  // All City Filter form
  dailyUserForm!: FormGroup;
  ...
  constructor(
    public service: DailyUserService,
  ...
  ngOnInit(): void {
     ...
     */
    this.dailyUserForm = this.formBuilder.group({
  ...
  export() {
    ...
    /* save to file */
    XLSX.writeFile(wb, 'Daily-User-Report.xlsx');
  }
}
