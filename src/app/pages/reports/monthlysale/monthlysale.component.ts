...
import {
  ...
  SortEvent,
} from './monthlysale-sortable.directive';
import { MonthlySale } from './monthlysale.model';
import { MonthlySaleService } from './monthlysale.service';
import * as XLSX from 'xlsx';

@Component({
  selector: 'app-monthlysale',
  templateUrl: './monthlysale.component.html',
  styleUrls: ['./monthlysale.component.scss'],
  providers: [MonthlySaleService, DecimalPipe],
})

/**
 * MonthlySale Component
 */
export class MonthlySaleComponent implements OnInit {
  ...
  hideme: boolean[] = [];
  tables$!: Observable<MonthlySale[]>;
  total$!: Observable<number>;

  company: any;
  getCompanys!: Observable<any>;
  companysCollection!: AngularFirestoreCollection<MonthlySale>;

  tableData!: MonthlySale[];
  totalData!: number;

  // All City Filter form
  monthlySaleFilterForm!: FormGroup;
  ...
  constructor(
    public service: MonthlySaleService,
  ...
  ngOnInit(): void {
     ...
     */
    this.monthlySaleFilterForm = this.formBuilder.group({
  ...
  onSubmitSearch(value: any) {
    ...
    if (this.monthlySaleFilterForm.invalid) {
      return;
    } else {
  ...
  export() {
    ...
    /* save to file */
    XLSX.writeFile(wb, 'Monthly-Sale-Report.xlsx');
  }
}
