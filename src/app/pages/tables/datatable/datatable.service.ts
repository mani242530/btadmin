...
const compare = (v1: string | string[], v2: string | string[]) =>
  v1 < v2 ? -1 : v1 > v2 ? 1 : 0;
...
function matches(table: Table, term: string, pipe: PipeTransform) {
  return (
    ...
    (table && table.vehicleType.toString().toLowerCase().includes(term)) ||
    (table && table.mobileNumber.includes(term)) ||
    (table && table.location.toLowerCase().includes(term)) ||
    (table &&
      table.serviceProvidedLocation.toString().toLowerCase().includes(term)) ||
    (table && table.referenceName.toLowerCase().includes(term)) ||
    (table && table.paymentStatus.toLowerCase().includes(term)) ||
    (table && table.accountStatus.toLowerCase().includes(term))
  );
}
...
export class DataTableService {
  ...
  private _search(): Observable<SearchResult> {
    ...
    // 1. sort
    let tables = sort(this.tableDataSnapshot, sortColumn, sortDirection);
